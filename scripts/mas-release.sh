...
#   ./scripts/mas-release.sh 1.9.1           # 버전 올려 .pkg 만들기
#   ./scripts/mas-release.sh 1.9.1 --upload  # 위 + App Store Connect 업로드
#
# 하는 일:
#   1) (버전 인자 있으면) project.yml 의 MARKETING_VERSION/CURRENT_PROJECT_VERSION 올림
#   2) xcodegen generate → oh-my-grid-mas 스킴 아카이브(자동 서명, 프로파일 자동 발급)
#   3) app-store-connect 방식으로 export → 서명된 .pkg
#   4) --upload: xcrun altool 로 App Store Connect 업로드
#
# 🔑 준비물 (1회):
#   - Apple Distribution + 3rd Party Mac Developer Installer 인증서.
#   - App Store Connect 에 앱 레코드 (번들 ID com.goldenrabbit.ohmygrid.mas).
#     레코드가 없으면 업로드가 "no app with bundle id" 로 거부된다.
#   - 업로드 인증(ASC API 키):
#       export OMG_ASC_API_KEY=<Key ID>       # ~/.appstoreconnect/private_keys/AuthKey_<KeyID>.p8
#       export OMG_ASC_API_ISSUER=<Issuer ID>
#
...
SCHEME="oh-my-grid-mas"
PROJECT="oh-my-grid.xcodeproj"
APP_NAME="oh-my-grid.app"
PKG_NAME="oh-my-grid.pkg"
TEAM_ID="M7NU9F8CZN"
BUNDLE_ID="com.goldenrabbit.ohmygrid.mas"
DD="$ROOT/build/masdd"
...
if [ -z "${OMG_ASC_API_KEY:-}" ] || [ -z "${OMG_ASC_API_ISSUER:-}" ]; then
  echo
  echo "ℹ️ App Store Connect API 키가 설정되지 않아 CLI 업로드를 건너뜁니다."
  echo "   아래를 설정한 뒤 다시 실행하세요:"
  echo "     export OMG_ASC_API_KEY=<Key ID>      # ~/.appstoreconnect/private_keys/AuthKey_<KeyID>.p8"
  echo "     export OMG_ASC_API_ISSUER=<Issuer ID>"
  exit 0
fi

echo "▸ 검증 (altool --validate-app)"
xcrun altool --validate-app -f "$PKG" -t macos \
  --apiKey "$OMG_ASC_API_KEY" --apiIssuer "$OMG_ASC_API_ISSUER"
