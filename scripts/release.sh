#!/usr/bin/env bash
#
# oh-my-grid 배포 빌더 (Sparkle 자동 업데이트 포함).
#
...
#    (프로필명을 바꾸면 OMOG_NOTARY_PROFILE 환경변수로 지정)
#
...
#  - Developer ID Application 인증서로 서명하면 고정 Team ID 가 신원이 되어,
#    같은 팀으로 서명한 모든 업데이트에서 TCC(손쉬운 사용) 권한이 유지된다.
...
#    codesign --deep 재서명에만 기대지 않는다.
DEV_ID="Developer ID Application"
NOTARY_PROFILE="${OMOG_NOTARY_PROFILE:-oh-my-opensnap}"
...
if ! security find-identity -v -p codesigning 2>/dev/null | grep -q "$DEV_ID"; then
  ...
  echo "  Apple Developer Program 멤버십 활성 후, Xcode → Settings → Accounts →"
  echo "  Manage Certificates → '+' → 'Developer ID Application' 로 발급하세요."
  exit 1
fi
SIGN=(codesign --force --options runtime --timestamp --sign "$DEV_ID")
SIGN_PRESERVE=(codesign --force --options runtime --timestamp --sign "$DEV_ID" --preserve-metadata=entitlements)
...
if [ -d "$FW" ]; then
  ...
  for nested in \
    ...
    "$B/Updater.app"; do
    [ -e "$nested" ] && "${SIGN_PRESERVE[@]}" "$nested"
  done
  [ -e "$B/Autoupdate" ] && "${SIGN[@]}" "$B/Autoupdate"
  [ -e "$B/Sparkle" ] && "${SIGN[@]}" "$B/Sparkle"
  "${SIGN_PRESERVE[@]}" "$FW"
fi
"${SIGN[@]}" "$APP"
...
DMG="$DIST/oh-my-grid-$VERSION.dmg"
...
ZIP="$DIST/oh-my-grid-$VERSION.zip"
...
# 릴리스 노트: CHANGELOG.md 의 "## <버전>" 섹션을 읽어 양쪽(appcast/GitHub)에 보여준다.
NOTES_MD=""
...
<rss version="2.0" xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle" xmlns:dc="http://purl.org/dc/elements/1.1/">
  <channel>
    ...
    <item>
      ...
      <description><![CDATA[<h2>oh-my-grid $VERSION</h2>
