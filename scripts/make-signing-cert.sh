#!/usr/bin/env bash
#
# oh-my-grid 배포 서명용 "자체서명 코드서명 인증서"를 만든다.
#
#   ./scripts/make-signing-cert.sh          # 키체인에 'oh-my-grid' 인증서 생성
#   ./scripts/make-signing-cert.sh --backup # 위 + 백업용 .p12 내보내기(암호 입력)
#
# 왜 필요한가:
#   ad-hoc 서명은 빌드마다 cdhash 가 바뀌어 macOS 26 에서 접근성 권한이
#   업데이트마다 풀린다. 고정된 인증서로 서명하면 권한이 유지된다.
#
# ⚠️⚠️ 매우 중요 ⚠️⚠️
#   이 인증서(개인키 포함)는 이 Mac 키체인에만 있다. 분실하면 새로 만들어야 하고,
#   새 인증서는 서명 식별자(Designated Requirement)가 달라서 **기존 사용자가 접근성
#   권한을 한 번 다시 켜야 한다**. 반드시 --backup 으로 .p12 를 받아 안전한 곳
...
if security find-identity 2>/dev/null | grep -q "\"$NAME\""; then
  ...
  openssl pkcs12 -export -inkey "$TMP/key.pem" -in "$TMP/cert.pem" -out "$TMP/id.p12" \
    -passout pass:omg -name "$NAME" \
    -legacy -macalg sha1 -keypbe PBE-SHA1-3DES -certpbe PBE-SHA1-3DES 2>/dev/null
  security import "$TMP/id.p12" -k ~/Library/Keychains/login.keychain-db -P omg -T /usr/bin/codesign -A
  rm -rf "$TMP"
  echo "✓ 생성 완료"
fi
...
if [ "${1:-}" = "--backup" ]; then
  OUT="$HOME/oh-my-grid-signing-cert-BACKUP.p12"
